...
causal_survival_forest <- function(X, Y, W, D,
  ...
  if (target == "RMST" && any(C.Y.hat <= 0.05)) {
    ...
    warning(paste("HI, THIS IS BORJA HERE! Estimated censoring probabilities go as low as:", round(min(C.Y.hat), 5),
    ...
    warning(paste("HI, THIS IS BORJA HERE! Estimated censoring probabilities are lower than 0.05",
